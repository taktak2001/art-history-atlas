...
import type { NetworkSemanticLevel } from '@/lib/network-map-layout';
...
export const NETWORK_MODE_PRESET: Record<NetworkMode, NetworkModePreset> = {
  overview: { lod: 'core', zoom: 0.78, relationLabels: false },
  study: { lod: 'standard', zoom: 1, relationLabels: true },
  focus: { lod: 'detailed', zoom: 1.24, relationLabels: true },
};

/**
 * 図に出す情報の段階。
 * かつては倍率から求めていたが、倍率はカメラ、段階は閲覧目的として分離した。
 */
export const SEMANTIC_LEVEL_BY_MODE: Record<NetworkMode, NetworkSemanticLevel> = {
  overview: 'overview',
  study: 'study',
  focus: 'detail',
};
