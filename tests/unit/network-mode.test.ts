...
describe('network mode', () => {
  ...
  it('モードごとに収録範囲とカメラ倍率が決まる', () => {
    ...
    // 倍率を決め打ちするのはSTUDYだけ。
    // OVERVIEWは収録全体へ、FOCUSは選択とその直接関係へ合わせる
    expect(NETWORK_MODE_PRESET.overview.camera).toBe('fit-all');
    expect(NETWORK_MODE_PRESET.focus.camera).toBe('fit-selection');
    expect(typeof NETWORK_MODE_PRESET.study.camera).toBe('number');
  });
