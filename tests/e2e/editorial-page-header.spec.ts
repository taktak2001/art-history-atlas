...
test('ムーブメント検索だけが固定され、フィルターと並び順はスクロールする', async ({
  page,
}) => {
  ...
  await expect(toolbar).toHaveAttribute('data-scroll-role', 'sticky-search');
  await expect(controls).toHaveAttribute('data-scroll-role', 'flow-controls');
  ...
  expect(await controls.evaluate((element) => getComputedStyle(element).position)).toBe('static');
  ...
  const controlsY = await controls.evaluate((element) => element.getBoundingClientRect().bottom);
  ...
  await expect(page.getByRole('button', { name: /詳細条件・フィルター/ })).not.toBeInViewport();
  await expect(page.getByRole('combobox', { name: '並び順' })).not.toBeInViewport();
});
