...
test('表示段階は倍率ではなくモードが決め、FOCUSでArtist・Work・Contextまで出す', async ({
  page,
}) => {
  const graph = page.getByRole('group', { name: '美術運動の関係ネットワーク図' });

  await page.goto('/network/?mode=overview&focus=mono-ha&scope=focus');
  ...
  await page.goto('/network/?mode=study&focus=mono-ha&scope=focus');
  await expect(graph).toHaveAttribute('data-network-semantic-level', 'study');
  await expect(graph.locator('[data-network-entity="artist"]').first()).toBeVisible();
  await expect(graph.locator('[data-network-entity="work"]')).toHaveCount(0);

  await page.goto('/network/?mode=focus&focus=mono-ha&scope=focus');
...
test('関係ラベルは密集しても互いに重ならず、全件表示される', async ({ page }) => {
  ...
  // ラベルは消さず、線が文字を貫通しないよう紙色のハローで抜く
  expect(result.haloCount).toBe(result.labelCount);
});

test('全関係を出してもラベル同士が重ならない', async ({ page }) => {
  await page.goto('/network/?mode=study&scope=all');
