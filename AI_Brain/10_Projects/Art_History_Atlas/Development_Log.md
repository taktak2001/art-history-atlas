...
## 記号の役割を統一（2026-08-09・PR #75）

「＋」は追加操作にも見えるため、アコーディオンの開閉はシェブロンへ統一した。

### サイト全体の記号ルール（今後これに従う）

| 記号 | 用途 |
|---|---|
| `›` / `⌄`（シェブロン） | アコーディオンの開閉。閉=右向き、開=90度回転して下向き |
| `＋` | 項目の追加など、実際に何かを加える操作**だけ** |
| `→` | 別ページへの遷移 |
| `×` | 削除・解除・閉じる |

- 共通コンポーネント `src/components/AccordionChevron.tsx`（SVG・`aria-hidden`）と
  `.accordion-chevron`（CSS）を新設。回転180ms、`prefers-reduced-motion` で無効化。
  丸背景・ボタン枠は付けない。最小タップ領域は行全体で44px以上。
- 置換した箇所:
  - About「分類の基準」7項目（ClassificationAccordion）
  - About「関係の基準」＝「関係データの読み方」＋関係タイプ9項目（RelationshipStandards）
  - ムーブメント一覧「詳細条件」（MovementsExplorer。PR #68 で自分が＋/−を入れていた）
  - 関係ネットワーク「線の見方」（NetworkGraph。`ⓘ` を使っていた `<details>`）
- **対象外と判断したもの（監査結果）**:
  - Timeline のズーム `＋/−`: 拡大縮小の操作でアコーディオンではない（一般的な慣習）。
  - Matrix のセル内 `+N`: 「あとN件を表示」で件数を伴う。開いた状態は「折りたたむ」と文字表示。
    シェブロンにすると件数が失われるため据え置き。**要判断として記録**。
  - Matrix の空セル `−`: 「データなし」の表示であり操作記号ではない。
  - ヘッダーの「メニューを開く」: ナビゲーション開閉でアコーディオンではない。
- E2E `tests/e2e/accordion-chevron.spec.ts` を追加（＋不在・シェブロン数・aria-expanded/controls・
  aria-hidden・44pxタップ領域・回転 matrix(0,1,-1,0,0,0)・Enter/Space操作・他ページ）。
- 検証: unit **249** / Playwright+axe **276 passed**。

## 「学説上の注意」の全件監査（2026-08-09・PR #76）

### 構造上の発見（重要）

詳細ページの「学説上の注意」ボックスは、実体が **`dates.note`（年代の範囲説明）** だった。
45件が「XからYまで」といった収録年代の説明であり、研究上の留保ではないのに
一律の「学説上の注意」という抽象的な見出しで表示されていた。これが
「何について注意しているのか分からない」原因。

### 対応

- スキーマに **`ScholarlyNote`**（`heading` / `body` / `sourceIds`）を追加し、
  `Movement.scholarlyNote?` として持たせた。`dates.note` は年代説明のまま役割を分離。
- データは `src/data/expansion/scholarly-notes.ts` にオーバーレイとして実装
  （117KBの movements.ts を直接編集せず、`applyScholarlyNotes` で最後に付与）。
- **見出しを16種類へ具体化**: 系譜について／名称について／影響関係について／地域区分について／
  運動としてのまとまり／参加の範囲について／作者の帰属について／制作の担い手について／
  内部の差異について／担い手について／収録範囲について／起点について／位置づけについて／
  年代の幅について／年代の精度について／系譜の広がりについて。
- 本文は「よくある理解 → どこが正確でないか → どう捉えるのが適切か」の型。平均81字（最長101字）。
- 専門語は同じ文で言い換え（例: 私淑 →「過去の作品を手本として学び（私淑）」）。
- UI: 見出しは `scholarlyNote.heading`。留保が無く年代説明だけの19件は「**年代の扱い**」として表示。
  ボックスの見た目（細い左罫線・薄い背景・警告色なし）は従来どおり維持。
- 検証を追加（`validate-data.ts` / unit）: 見出しに「学説上の注意」を使わない、
  「〜を扱う」だけの編集方針を残さない、220字上限、sourceIds はそのムーブメントに
  紐づく既存出典のみ。
- 監査レポート `docs/scholarly-note-audit.md` を `scripts/gen-scholarly-note-audit.ts` で自動生成。

### 出典に無い説明は作らない（未確認事項）

- 印象派の「名称は当初は揶揄的な呼称」は、既存出典が明示していないため **追記していない**。
  ご提示の修正例にもあったが、出典で確認できるまで保留。
- 影響関係の限定（アルテ・ポーヴェラ↔もの派）は、既存出典が両者を扱う範囲でのみ記述。
- 検証: unit **259** / Playwright+axe **276 passed**。

### 追記（2026-08-09・PR #77）: ラベル可読性の仕上げ（差分のみ）

同趣旨の指示を再受領。大半は PR #71〜#74 で実装済みだったため、**未達の差分だけ**を修正した。

**既に満たしていた項目**: 候補位置による衝突回避 / 優先順位（選択直結→1ホップ→その他）/
paper色バックプレート（枠線・影なし）/ semanticな色・線種の維持 / 視覚階層 /
`useMemo` による再計算（毎フレーム計算なし）/ 「関係一覧」の補助UI /
ラベル層の `pointer-events: none`（スワイプ・panを妨げない）。

**今回直した差分**:
- フォントサイズ **desktop 10px → 12px**（mobile 11→12）。desktopがmobileより小さい状態だった。
- 太さ **700 → 600**（medium〜semibold）。
- **矢印との最小すき間 0 → 4px**（`LABEL_GAP.arrow`）。従来 arrow の gap だけ 0 だった。
- 引き出し線の**長さペナルティ**（`leaderLength: 3`）を追加。距離ペナルティは40pxで頭打ちのため、
  56pxと140pxが同点になり長い方が選ばれることがあった。短い候補を優先させる。

**残る трейд-off（正直な報告）**:
フォントを12pxへ上げた結果、密な列では引き出し線の使用が増える（ミニマリズムで6本・72〜96px）。
「ブロックに重ならない」を最優先する既存方針とノード列の密度（ノード幅148〜166px・行間84〜88px）
の組み合わせにより、短い位置は全てノードに重なるため。フォントを12→10へ戻せば引き出し線は減るが、
可読性の指示（desktop 12〜13px・縮小より配置で解く）に反するため12pxを採用した。
根本解決にはノード列レイアウト（行間・列幅）の調整が必要。
- 検証: unit **259** / Playwright+axe **276 passed**。

## 密集列の局所レイアウト調整（2026-08-09・PR #78）

PR #77 で残った「引き出し線が72〜96px級に伸びる」問題を、**ノード配置側**で解消した。
collision avoidance の実装（#71〜#74・#77）はそのまま維持。

### 原因

ミニマリズムは**1列に7ノードが行間84pxで積み上がる**構造で、
ラベルが入る隙間（84−58＝26px）が足りず、横へ逃がすとノード幅148〜166pxを
越えるまで離れる必要があった。フォント12px化でさらに圧迫されていた。

### 対応（局所のみ・全体一律には広げない）

`layout` useMemo に密集列の判定と局所調整を追加。
- 判定: 同一列を縦走する関係が3本以上、または選択ノードの直接関係が4本以上でその列が関与
- 密集列のみ **行間 +20px（desktop）/ +16px（mobile）**
- 密集列のみ **x を左右交互に ±12px（desktop）/ ±8px（mobile）** ずらす
- 列構造・時代/地域の意味配置は不変。規則的な交互オフセットのみ

副次効果として、`getSharedCorridorOffset`（同一x前提）が隣接行では発火しなくなり、
**edge fan-out が自動的に必要最小限**へ収まった（仕様6の要求と一致）。

### 実測（before = PR #77 / after = 本PR）

| fixture・幅 | before 引き出し線 | after 引き出し線 | 衝突 |
|---|---|---|---|
| ミニマリズム 375px | 6本 / 最大96 / 平均88 | **1本 / 最大56 / 平均56** | 0 |
| ミニマリズム 1280px | 6本 / 最大96 | **0本** | 0 |
| イタリア・ルネサンス 375px | 1本 / 118 | **0本** | 0 |
| イタリア・ルネサンス 1280px | 1本 / 118 | **0本** | 0 |

行間: 84→100（mobile）/ 88→108（desktop、密集列のみ）。
縦の伸びはミニマリズムで canvasH 約740→880（+19%、密集時のみ）。
label-label / label-node / viewport overflow はすべて 0 を維持。
...
## MovementPickerの再利用 Phase 1: Network（2026-08-10・PR #80）

PR #79 の MovementPicker を汎用化し、**ネットワークのフォーカス選択**へ適用した。
ホーム検索は指示どおり変更していない。

- Picker に `mode: 'select' | 'navigate'` と `closeOnSelect` を追加（forkはしない）。
  navigate では選択件数・上限・完了ボタンを出さず、1件選ぶと閉じる。
  検索インデックス・時代別グループ化・別名/作家/作品検索・キーボード操作・
  アクセシビリティは共通のまま。
- Network に「ムーブメントを検索」トリガーを追加（**既存のムーブメント選択UIは無く、新規追加**。
  948行目の select は関係タイプのフィルタで別物）。
- `focusMovement(id)`: URL由来の focus 到着と同じ経路に載せ、PR #70 の自動調整を再利用する。
  `urlFocusRef` を設定し `focusAutoAppliedForRef` を戻すことで、
  「最小LODへ自動調整 → scope=このムーブメント → 1ホップへ一度だけ寄せる」が1回だけ走る。
- 実測: 「ピカソ」→ キュビスム選択で `?lod=standard&focus=cubism&scope=focus`、
  「充実」へ自動調整、直接関係4件・4ノード表示。その後 LOD を手動で「基本」に戻しても
  focus と scope は保持され、自動設定へ戻らない。
- 検証: unit **279** / Playwright+axe **290 passed**。

### Phase 2（Timeline）は未着手のまま戻した — 理由

タイムラインへの「ムーブメントへ移動」も実装したが、**動作が不完全なため revert した**。
判明した障害:
1. `scrollToYear` が使う年代→x は**軸内の座標**で、スクロール要素は地域ラベル列を含むため
   座標系がずれる。時代ジャンプは focusYear が時代の先頭（x≈0）なので露見していなかった。
2. モード変更時に既存の `[modeId]` 効果が `pendingJump` を見て**左端へ戻す**ため、
   後続のスクロールと競合する。
3. `[data-timeline-bar="surrealism"]` が**DOM上に2つ存在**し（バーの二重描画）、
   要素基準で寄せる方式でも「どちらを基準にするか」が定まらない。

モード切替と一時強調までは動いていたが、水平位置合わせが未達。
HorizontalTimeline のDOM構造（二重描画の意図）を確認してから再着手する。
